...
#
# DORMANT (2026-08-11): binds :8001 and is invoked ONLY by model-router.py, which was
# RETIRED when vLLM moved to serving :8000 directly (live ExecStart = serve-tqk8v4-fg.sh,
# which is the byte-identical --port 8000 variant). Nothing calls this on the live path.
# KEEP IT — it is part of the router ROLLBACK path:
#   cp serve-tqk8v4-fg.sh.bak-router-retired-20260811 serve-tqk8v4-fg.sh && sudo systemctl restart vllm-qwen27b
# Do NOT repoint this to :8000 — the direct-serve script owns :8000; the router needs this on :8001.
