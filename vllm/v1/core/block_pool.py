    ...
    def cache_full_blocks(
        ...
        Args:
            ...
            block_mask: Optional mask aligned with
                ...
                blocks). Used by groups with sparse retention (e.g. Mamba
                with ``VLLM_PREFIX_CACHE_RETENTION_INTERVAL`` set), so
                intermediate state snapshots stay out of the prefix-cache
                hash map.
        """
        ...
        for i, blk in enumerate(new_full_blocks):
            # Some blocks may be null or masked out when enabling sparse
            # attention like sliding window attention, or Mamba models with
            # prefix-caching in align mode (or sparse retention). We skip
            # null/masked-out blocks here.
